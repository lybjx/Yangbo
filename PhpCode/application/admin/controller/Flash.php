...
class Flash extends Backend
{
    ...
    public function index()
    {
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model->table("cms_video")
                ->where($where)
                ->order("id", $order)
                ->count();

            $list = $this->model->table("cms_flash")
                ->where($where)
                ->order("id", $order)
                ->limit($offset, $limit)
                ->select();
            ...
            if ($this->request->isPost()) {
                ...
                if ($params) {
                    ...
                    try {
                        ...
                        $result = $News->table("cms_flash")->insertGetId($params);
                        if ($result !== false) {
    ...
    public function edit($ids=null)
    {
            ...
            if ($this->request->isPost()) {
                ...
                if ($params) {
                    ...
                    try {
                        ...
                        $News=new \app\common\model\News();
                        $News->table("Yb_cms_db");
                        $result = $News->table("cms_flash")->update($params);
                        if ($result !== false) {
        ...
        $this->model=model('News');
        $articledetail = $this->model->table("cms_flash")->where(['id' => $ids])->find();
    ...
    public function del($ids=0){
        if ($this->request->isAjax())
        {
            try {
                $News=new \app\common\model\News();
                $result = $News->table("cms_flash")->where("id=".$ids)->delete();
